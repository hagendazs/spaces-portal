...
>
> **Scope note (updated 2026-06-25):** the static site is now **built and deployed** — `site/index.html` + `site/solutions/finance.html`, live at https://hagendazs.github.io/spaces-portal/ . The on-page AEO work in this doc (static render, JSON-LD, robots, `llms.txt`, sitemap, semantic markup) is **shipped** — see the checked items in §5. The score below was authored pre-build as a readiness target; actual on-page readiness now exceeds it. Remaining gap is **off-page only**: real production domain (`spaces.sks.com.tw`), third-party mentions, full English parity, and live measurement.
>
> **IA note:** this doc specifies the **target 5-page nav** (首頁/解決方案/產品介紹/FAQ/媒體報導). The current build is an **MVP**: a single-page home (all sections inline) + one scenario page (`solutions/finance.html`) as the replicable master template. The remaining 4 nav pages + 5 scenario verticals are the next phase.
...
- [x] Static/SSR render — content present in raw HTML, not JS-only. *(static HTML, verified JS-disabled)*
- [x] `robots.txt`: allow `GPTBot`, `OAI-SearchBot`, `ClaudeBot`, `PerplexityBot`, `Google-Extended`, `Bingbot`. *(+`ChatGPT-User`)*
- [x] `llms.txt` at root → links to Solutions + FAQ pages.
- [x] JSON-LD: `Organization` (sitewide) · `FAQPage` (FAQ + each scenario) · `Product` (each device) · `HowTo` (安裝) · `BreadcrumbList`. *(home: 5 blocks; finance: BreadcrumbList+Service+FAQPage)*
- [x] Every page: `<title>`, `<meta description>` (= the 「一句話」), `dateModified`, 新光保全 attribution.
- [x] Semantic markup: spec/benefit tables as `<table>`, 效益 as `<ul>`, questions as `<h2>`.
- [x] `sitemap.xml` with real `lastmod`. *(home + finance; on `spaces.sks.com.tw` canonical domain)*
- [ ] Measurement: tag referrals from `chat.openai.com`/`perplexity.ai`/`gemini.google.com`; reuse slide-1 QR tracking. *(off-page — pending)*
- [ ] (Staged) English parity for the 6 scenario + FAQ pages — biggest lever on dim 8.
- [ ] Replicate finance master → 零售／倉儲／辦公／社區／住家 (5 scenario pages) + 產品介紹／FAQ／媒體報導 nav pages.
```
